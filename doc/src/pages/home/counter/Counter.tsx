...
import * as counter from "./Counter.module.css";
	...
	return (
		<div class={clsx(counter["wrapper"])}>
			...
			<span class={clsx(counter["description"])}>
				It was hydrated on the client with an initial value of{" "}
				<strong>{initialValue}</strong> supplied by the server
			</span>
			...
			<div class={clsx(counter["counter"])}>
				<button
					...
					onClick={() => {
						globalCount.value = Math.max(0, count.value - 1);
					}}
				>
					decrement
				</button>
				...
				<button
					...
					class={clsx(counter["button"])}
					onClick={() => {
						globalCount.value = count.value + 1;
					}}
				>
					increment
				</button>
