...
import { TOC } from "../../data/doc/toc.ts";
	...
	for (const [version, versionToc] of Object.entries(TOC)) {
		for (const [lang, langToc] of Object.entries(versionToc.langs)) {
			for (const entry of langToc.entries) {
				paths.push({ lang: lang, version: version, slug: entry.slug.split("/") });
			}
			paths.push({ lang: lang, version: version });
		}
	}
...
export async function build({
	params: { slug = ["introduction"], version, lang },
