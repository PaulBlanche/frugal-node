...
import * as readableStream from "../../../packages/frugal/src/utils/readableStream.js";

test("unit/frugal/utils/readableStream.js: read string stream", async () => {
	const stream = /** @type {webStream.ReadableStream<string>} */ (
		/** @type {any} */ (webStream.ReadableStream).from(["1", "2", "3"])
	);
	assert.strictEqual(await readableStream.readStringStream(stream), "123");
});

test("unit/frugal/utils/readableStream.js: read binary stream", async () => {
...
test("unit/frugal/utils/readableStream.js: text line stream", async () => {
	const stream = /** @type {webStream.ReadableStream<string>} */ (
		/** @type {any} */ (webStream.ReadableStream).from([
			"foo foo\nba",
			"r\nbaz b",
...
test("unit/frugal/utils/readableStream.js: merge streams with error", async () => {
	const stream1 = /** @type {webStream.ReadableStream<string>} */ (
		/** @type {any} */ (webStream.ReadableStream).from(
			(async function* () {
