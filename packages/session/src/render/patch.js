...
/**
 * @param {import("./types.ts").NodePatch} patch
 * @param {Node} target
 */
export function patch(patch, target) {
	/** @type {import("./patch.ts").PatchQueueItem[]} */
	const queue = [];
	queue.push({
		patch,
		parent: target,
	...
	/** @type {import("./patch.ts").PatchQueueItem | undefined} */
	let current;
 ...
 * @param {import("./types.ts").AttributePatch[]} patches
 */
