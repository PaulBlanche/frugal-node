...
export type Collapse<OBJECT extends PathObjectBase> = {
...
type Consume<CONTEXT extends ContextBase> = CONTEXT["rest"] extends `${infer CHAR}${infer REST}`
	? CONTEXT["escape"] extends true
		...
		: CHAR extends ":"
		  ? Consume<ConsumeNamedParameter<CONTEXT>>
		  : CHAR extends "("
			  ? Consume<ConsumeUnnamedParameter<CONTEXT>>
			  : CHAR extends "{"
				  ? REST extends `${infer PARAM}}${infer REST}`
						? Consume<{
								rest: `${PARAM}${REST}`;
								index: CONTEXT["index"];
								object: CONTEXT["object"];
...
type ConsumeNamedParameter<CONTEXT extends ContextBase> = ConsumeModifier<
	ConsumePattern<
		ConsumeParameterName<{
			rest: CONTEXT["rest"];
			index: CONTEXT["index"];
		}>
	>
...
type ConsumeUnnamedParameter<CONTEXT extends ContextBase> = ConsumeModifier<
	ConsumePattern<CONTEXT>
> extends { name: infer NAME; optional?: infer OPTIONAL; rest: infer REST; index: infer INDEX }
	? NAME extends string | number
		? {
				object: CONTEXT["object"] &
					(OPTIONAL extends true
...
type ConsumeModifier<INPUT extends ParameterContextBase> = INPUT["rest"] extends `?${infer REST}`
	? {
			name: INPUT["name"];
			index: INPUT["index"];
			optional: true;
...
type ConsumePattern<INPUT extends ParameterContextBase> =
